    ...
    public class AutoUpdateService : IDisposable
    {
        ...
        private async Task<bool> LaunchInstallerAndExitAsync(string installerPath)
        {
            try
            {
                ...
                // 2. Lancer l'installateur en mode TRES silencieux (aucune fenetre)
                // 3. Relancer l'application
                string batchPath = Path.Combine(UPDATE_FOLDER, "update_launcher.bat");
                string appPath = Assembly.GetExecutingAssembly().Location;

                // Script batch totalement silencieux - aucune fenetre visible
                // /VERYSILENT = pas de fenetre du tout (seulement UAC si necessaire)
                // /SUPPRESSMSGBOXES = pas de message boxes
                // /NORESTART = on gere le restart nous-memes
...
";
                ...
                // Lancer le batch en TOTALEMENT CACHE (pas de fenetre visible)
