...
namespace XnrgyEngineeringAutomationTools.Services
{
    ...
    public class DeviceTrackingService : IDisposable
    {
        ...
        /// <summary>
        /// Obtient l'URL Firebase avec le token d'authentification Service Account
        /// SECURISE - Utilise FirebaseAuthService pour ajouter access_token
        /// </summary>
        private async Task<string> GetAuthenticatedUrlAsync(string path)
        {
            string baseUrl = $"{FIREBASE_DATABASE_URL}/{path}.json";
            return await FirebaseAuthService.GetAuthenticatedUrlAsync(baseUrl);
        }
            ...
            try
            {
                ...
                var deviceInfo = new
                {
                    ...
                    software = new
                    {
                        ...
                        xeatInstallPath = GetXeatInstallPath(),
                    ...
                    {
                        [$"install_{DateTimeOffset.UtcNow.ToUnixTimeMilliseconds()}"] = new
                        {
                            action = "install",
                            ...
                            machineName = Environment.MachineName,
                            installPath = GetXeatInstallPath(),
                            osVersion = GetOSName(),
                            success = true,
                ...
                // SECURISE: Utilise l'URL authentifiee avec Service Account
                string url = await GetAuthenticatedUrlAsync($"devices/{_deviceId}");
        ...
        private async Task SendHeartbeatAsync()
        {
            ...
            try
            {
                ...
                // SECURISE: PUT sur /devices/{id}/heartbeat avec authentification
                string heartbeatUrl = await GetAuthenticatedUrlAsync($"devices/{_deviceId}/heartbeat");
                await _httpClient.PutAsync(heartbeatUrl, content);
                ...
                string onlineUrl = await GetAuthenticatedUrlAsync($"devices/{_deviceId}/status/online");
                await _httpClient.PutAsync(onlineUrl, onlineContent);
                
                var lastSeenContent = new StringContent($"\"{nowUtc}\"", Encoding.UTF8, "application/json");
                string lastSeenUrl = await GetAuthenticatedUrlAsync($"devices/{_deviceId}/status/lastSeen");
                await _httpClient.PutAsync(lastSeenUrl, lastSeenContent);
        ...
        public async Task UnregisterDeviceAsync()
        {
            ...
            try
            {
                ...
                // SECURISE: Mettre a jour status.online = false avec authentification
                ...
                // SECURISE: Mettre a jour heartbeat.status = offline avec authentification
                ...
                string heartbeatUrl = await GetAuthenticatedUrlAsync($"devices/{_deviceId}/heartbeat");
                await _httpClient.PutAsync(heartbeatUrl, heartbeatContent);
            ...
            try
            {
                ...
                using (var key = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Microsoft\Windows NT\CurrentVersion"))
                {
                    if (key != null)
                    {
                        ...
                        // Windows 11 detection: Build >= 22000 = Windows 11
                        // Le registre peut afficher "Windows 10" meme sur Windows 11
                        if (int.TryParse(currentBuild, out int buildNumber) && buildNumber >= 22000)
                        {
                            // Corriger le nom du produit pour Windows 11
        ...
        /// <summary>
        /// Obtient le chemin d'installation de XEAT
        /// </summary>
        private string GetXeatInstallPath()
        {
            try
            {
                // D'abord essayer l'emplacement actuel de l'executable
                string currentExePath = System.Reflection.Assembly.GetExecutingAssembly().Location;
                if (!string.IsNullOrEmpty(currentExePath))
                {
                    return Path.GetDirectoryName(currentExePath) ?? "unknown";
                }
                
                // Fallback: emplacement standard
                string programFiles = Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles);
                return Path.Combine(programFiles, "XNRGY Climate Systems ULC", "XNRGY Engineering Automation Tools");
            }
            catch
            {
                return "unknown";
        ...
        /// <summary>
        /// Obtient le nom complet du systeme d'exploitation
        /// </summary>
        ...
        /// SECURISE: Utilise l'authentification Service Account
        /// </summary>
        private async Task IncrementStatisticAsync(string path)
        {
            try
            {
                // SECURISE: Lire la valeur actuelle avec authentification
                string readUrl = await GetAuthenticatedUrlAsync(path);
                var response = await _httpClient.GetAsync(readUrl);
                int currentValue = 0;
                if (response.IsSuccessStatusCode)
                {
                ...
                // SECURISE: Sauvegarder avec authentification
                string writeUrl = await GetAuthenticatedUrlAsync(path);
                var putContent = new StringContent(currentValue.ToString(), Encoding.UTF8, "application/json");
                await _httpClient.PutAsync(writeUrl, putContent);
            }
            catch { }
        }
        ...
        public async Task IncrementUploadsAsync()
        {
            ...
            {
                // SECURISE: Lire la valeur actuelle avec authentification
                string readUrl = await GetAuthenticatedUrlAsync($"devices/{_deviceId}/usage/totalUploads");
                var response = await _httpClient.GetAsync(readUrl);
                int currentValue = 0;
                if (response.IsSuccessStatusCode)
                {
                ...
                // SECURISE: Sauvegarder avec authentification
                string writeUrl = await GetAuthenticatedUrlAsync($"devices/{_deviceId}/usage/totalUploads");
                var putContent = new StringContent(currentValue.ToString(), Encoding.UTF8, "application/json");
                await _httpClient.PutAsync(writeUrl, putContent);
                ...
                string lastUploadUrl = await GetAuthenticatedUrlAsync($"devices/{_deviceId}/usage/lastUpload");
                await _httpClient.PutAsync(lastUploadUrl, lastUploadContent);
        ...
        public async Task IncrementModulesCreatedAsync()
        {
            ...
            {
                // SECURISE: Lire la valeur actuelle avec authentification
                string readUrl = await GetAuthenticatedUrlAsync($"devices/{_deviceId}/usage/totalModulesCreated");
                var response = await _httpClient.GetAsync(readUrl);
                int currentValue = 0;
                if (response.IsSuccessStatusCode)
                {
                ...
                // SECURISE: Sauvegarder avec authentification
                string writeUrl = await GetAuthenticatedUrlAsync($"devices/{_deviceId}/usage/totalModulesCreated");
                var putContent = new StringContent(currentValue.ToString(), Encoding.UTF8, "application/json");
                await _httpClient.PutAsync(writeUrl, putContent);
                ...
                string lastModuleUrl = await GetAuthenticatedUrlAsync($"devices/{_deviceId}/usage/lastModuleCreated");
                await _httpClient.PutAsync(lastModuleUrl, lastModuleContent);
        ...
        public async Task IncrementSessionsAsync()
        {
            try
            {
                // SECURISE: Lire avec authentification
                string readUrl = await GetAuthenticatedUrlAsync($"devices/{_deviceId}/usage/totalSessions");
                var response = await _httpClient.GetAsync(readUrl);
                int currentValue = 0;
                if (response.IsSuccessStatusCode)
                {
                ...
                // SECURISE: Sauvegarder avec authentification
                string writeUrl = await GetAuthenticatedUrlAsync($"devices/{_deviceId}/usage/totalSessions");
                var putContent = new StringContent(currentValue.ToString(), Encoding.UTF8, "application/json");
                await _httpClient.PutAsync(writeUrl, putContent);
