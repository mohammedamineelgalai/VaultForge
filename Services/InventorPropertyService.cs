...
namespace VaultAutomationTool.Services
{
    ...
    public class InventorPropertyService : IDisposable
    {
        ...
        /// <summary>
        /// Initialise la connexion à Inventor.
        /// IMPORTANT: Ne crée PAS de nouvelle instance - se connecte uniquement à une instance existante.
        /// Cela évite les instances multiples d'Inventor en arrière-plan.
        /// </summary>
        public bool Initialize()
        {
            try
            {
                ...
                // Essayer de se connecter à une instance existante UNIQUEMENT
                try
                {
                ...
                {
                    // Pas d'instance existante - NE PAS créer de nouvelle instance
                    // Cela évite les instances invisibles multiples qui restent en arrière-plan
                    Logger.Log("   [!] Aucune instance Inventor active - veuillez démarrer Inventor d'abord", Logger.LogLevel.WARNING);
                    return false;
                }
            }
            catch (Exception ex)
        ...
        public bool SetIProperties(string filePath, string? projectNumber, string? reference, string? module)
        {
            ...
            try
            {
                ...
                if (VaultFileHelper.IsVaultPath(filePath))
                {
