...
namespace XnrgyEngineeringAutomationTools.Services
{
    ...
    public class DeviceTelemetryService : IDisposable
    {
        ...
        /// Preserve la date d'enregistrement originale si le device existe deja
        /// </summary>
        public async Task RegisterDeviceAsync()
        {
            try
            {
                ...
                // Verifier si le device existe deja et recuperer registeredAt original
                string existingRegisteredAt = null;
                try
                {
                    var existingData = await ReadFirebaseAsync($"/devices/{_deviceId}/registration/registeredAt");
                    if (!string.IsNullOrEmpty(existingData) && existingData != "null")
                    {
                        // Enlever les guillemets du JSON string
                        existingRegisteredAt = existingData.Trim('"');
                        Logger.Log($"[i] Device existant - date installation preservee: {existingRegisteredAt}");
                    }
                }
                catch { }
                ...
                // Si le device existait deja, preserver la date d'enregistrement originale
                if (!string.IsNullOrEmpty(existingRegisteredAt))
                {
                    if (deviceData.ContainsKey("registration") && deviceData["registration"] is Dictionary<string, object> regData)
                    {
                        regData["registeredAt"] = existingRegisteredAt;
                    }
                }
        ...
        /// <summary>
        /// Lit des donnees depuis Firebase (GET)
        /// </summary>
        private async Task<string> ReadFirebaseAsync(string path)
        {
            try
            {
                string url = $"{FIREBASE_DATABASE_URL}{path}.json";
            ...
            {
                Logger.Log($"[!] Erreur Firebase GET {path}: {ex.Message}", Logger.LogLevel.DEBUG);
                return null;
            }
        }
