...
namespace XnrgyEngineeringAutomationTools.Services
{
    ...
    public class VaultSdkService
    {
        ...
        /// <summary>
        /// Decouvre les serveurs Vault disponibles sur le reseau
        /// Retourne la liste des serveurs Vault connus pour XNRGY
        /// </summary>
        /// <returns>Liste des noms de serveurs</returns>
        public List<string> DiscoverVaultServers()
        {
            var servers = new List<string>();
            
            try
            {
                Logger.Log("[>] Chargement des serveurs Vault connus...", Logger.LogLevel.INFO);
                
                // Liste des serveurs XNRGY connus (pas de methode SDK pour decouverte automatique)
                // L'API ConnectionManager ne dispose pas de GetAutodeskServers dans cette version
                var knownServers = new[] { "VAULTPOC", "LOCALHOST" };
                ...
                Logger.Log($"[+] {servers.Count} serveur(s) disponible(s)", Logger.LogLevel.INFO);
            }
            catch (Exception ex)
            {
                Logger.Log($"[-] Erreur chargement serveurs: {ex.Message}", Logger.LogLevel.ERROR);
                servers.Add("VAULTPOC");
            }
        ...
        /// <summary>
        /// Obtient la liste des Vaults disponibles sur un serveur
        /// Note: L'API ConnectionManager ne dispose pas de GetAutodeskVaults dans cette version du SDK
        /// On retourne les Vaults connus pour XNRGY
        /// </summary>
        /// <param name="server">Nom du serveur Vault</param>
        /// <returns>Liste des noms de Vaults</returns>
        public List<string> GetVaultNames(string server)
        {
            var vaults = new List<string>();
            
            try
            {
                Logger.Log($"[>] Chargement des Vaults pour {server}...", Logger.LogLevel.INFO);
                
                // Liste des Vaults XNRGY connus (pas de methode SDK pour enumeration)
                // PROD_XNGRY = Production, TestXNRGY = Test
                var knownVaults = new[] { "PROD_XNGRY", "TestXNRGY" };
                
                foreach (var vault in knownVaults)
                {
                    vaults.Add(vault);
                    Logger.Log($"   [+] Vault: {vault}", Logger.LogLevel.DEBUG);
                }
                
                Logger.Log($"[+] {vaults.Count} Vault(s) disponible(s) sur {server}", Logger.LogLevel.INFO);
            }
            catch (Exception ex)
            {
                Logger.Log($"[-] Erreur chargement Vaults pour {server}: {ex.Message}", Logger.LogLevel.ERROR);
                // Retourner au moins PROD_XNGRY
                vaults.Add("PROD_XNGRY");
            }
       ...
       public bool CleanVaultFolder(string vaultFolderPath)
{
    ...
    catch (Exception ex)
    {
                    ...
                    try
                    {
                        ...
                        try
                        {
                            ...
                            if (downloadResult.FileResults != null && downloadResult.FileResults.Any(r => r.LocalPath != null))
                            {
                                ...
                                VaultFileHelper.RemoveReadOnly(localFilePath);
                        ...
                        // Commentaire standardise (format simplifie)
                        string checkinCommentInv = !string.IsNullOrEmpty(checkInComment) 
                            ? checkInComment 
                            : $"[Auto] MAJ Proprietes | XEAT";
                            ...
                            if (updateCatMethod != null)
                            {
                                updateCatMethod.Invoke(documentServiceExtensions,
                                    new object[] {
                                        new[] { file.MasterId },
                                        new[] { categoryId },
                                        $"[Auto] MAJ Categorie | {categoryName} | XEAT"
                                    });
                                ...
                                try
                                {
                                    ...
                                    if (updateMethod != null)
                                    {
                                        updateMethod.Invoke(_connection.WebServiceManager.DocumentService, 
                                            new object[] { new[] { file.Id }, new[] { lifecycleStateId.Value }, "[Auto] Assignation lifecycle | XEAT" });
                                    ...
                                    if (updateRevisionMethod != null)
                                    {
                                        updateRevisionMethod.Invoke(documentServiceExtensions, 
                                            new object[] { 
                                                ...
                                                $"[Auto] MAJ Revision | {revision} | XEAT" 
                                            });
                                    ...
                                    if (existingFile != null && existingFile.MasterId > 0)
                                    {
                                        ...
                                        if (lifecycleDefinitionId.HasValue && lifecycleStateId.HasValue)
                                        {
                                            try
                                            {
                                                ...
                                                {
                                                    updateMethod.Invoke(_connection.WebServiceManager.DocumentService, 
                                                        new object[] { new[] { existingFile.Id }, new[] { lifecycleStateId.Value }, "[Auto] Assignation lifecycle | XEAT" });
                                                    Logger.Log($"   [+] Lifecycle assigne avec succes", Logger.LogLevel.INFO);
                                                }
                                            }
                                            catch (Exception lifecycleEx)
            ...
            try
            {
                ...
                using (FileStream fileStream = new FileStream(filePath, FileMode.Open, FileAccess.Read))
                {
                    ...
                    var addedFile = _connection!.FileManager.AddFile(
                        targetFolder,
                        fileName,
                        "[Auto] Upload initial | XEAT",  // Commentaire standardise
                        fileInfo.LastWriteTime,
                        null,
                        null,
                        fileClass,
        ...
        private bool ApplyPropertiesWithoutLifecycle(long fileMasterId, string? projectNumber, string? reference, string? module)
        {
            try
            {
                ...
                try
                {
                    ...
                    string checkinComment = $"[Auto] MAJ Proprietes | XEAT";
                    _connection.FileManager.CheckinFile(
                        vaultFile,
                        checkinComment,
                        false,
                    ...
                    foreach (var file in files)
                    {
                        try
                        {
                            ...
                            if (result.FileResults != null && result.FileResults.Any(r => r.LocalPath != null))
                            {
                                ...
                                // CRITIQUE: Retirer l'attribut ReadOnly apres telechargement depuis Vault
                                var localPath = result.FileResults.First().LocalPath?.FullPath;
        ...
        public bool AcquireFile(ACW.File file, string localPath, bool checkout = false)
        {
            ...
            try
            {
                ...
                if (result.FileResults != null && result.FileResults.Any(r => r.LocalPath != null))
                {
                    ...
                    {
                        VaultFileHelper.CopyFile(downloadedPath, localPath, true);
                    }
                    
                    // Retirer ReadOnly du fichier telecharge (fichiers Vault sont toujours ReadOnly)
                    VaultFileHelper.RemoveReadOnly(localPath);
        ...
        /// <summary>
        /// Met a jour un fichier Vault avec un nouveau contenu (texte/bytes)
        /// IMPLEMENTATION ROBUSTE avec 5 strategies de fallback:
        /// 1. AcquireFiles(Checkout) + CheckinFile avec chemin AcquireFiles
        /// 2. AcquireFiles(Checkout) + CheckinFile avec null
        /// 3. DocumentService.CheckoutFile + CheckinFile avec chemin local
        /// 4. DocumentService.CheckoutFile + CheckinFile avec null
        /// 5. Delete + AddFile (perd l'historique mais fonctionne toujours)
        /// </summary>
        public bool UpdateFileContentInVault(ACW.File file, string localFilePath, byte[] newContent, string comment)
        {
            if (!IsConnected || _connection == null || file == null || newContent == null)
                return false;

            Logger.Log($"[>] UpdateFileContentInVault: {file.Name} ({newContent.Length} bytes)", Logger.LogLevel.INFO);

            // STRATEGIE 0 (PRIORITAIRE): CheckinFile avec Stream - evite les problemes de chemin!
            if (TryStrategy0_CheckinWithStream(file, localFilePath, newContent, comment))
            {
                Logger.Log($"[+] Strategie 0 reussie: CheckinFile avec MemoryStream", Logger.LogLevel.INFO);
                return true;
            }

            // STRATEGIE 1: AcquireFiles(Checkout) + CheckinFile avec chemin AcquireFiles
            if (TryStrategy1_AcquireCheckout_CheckinWithPath(file, localFilePath, newContent, comment))
            {
                Logger.Log($"[+] Strategie 1 reussie: AcquireFiles + CheckinFile(path)", Logger.LogLevel.INFO);
                return true;
            }

            // STRATEGIE 2: AcquireFiles(Checkout) + CheckinFile avec null
            if (TryStrategy2_AcquireCheckout_CheckinNull(file, localFilePath, newContent, comment))
            {
                Logger.Log($"[+] Strategie 2 reussie: AcquireFiles + CheckinFile(null)", Logger.LogLevel.INFO);
                return true;
            }

            // STRATEGIE 3: DocumentService.CheckoutFile + CheckinFile avec chemin
            if (TryStrategy3_DocServiceCheckout_CheckinWithPath(file, localFilePath, newContent, comment))
            {
                Logger.Log($"[+] Strategie 3 reussie: DocService.Checkout + CheckinFile(path)", Logger.LogLevel.INFO);
                return true;
            }

            // STRATEGIE 4: DocumentService.CheckoutFile + CheckinFile avec null
            if (TryStrategy4_DocServiceCheckout_CheckinNull(file, localFilePath, newContent, comment))
            {
                Logger.Log($"[+] Strategie 4 reussie: DocService.Checkout + CheckinFile(null)", Logger.LogLevel.INFO);
                return true;
            }

            // STRATEGIE 5: Force Sync (UndoCheckout + Reacquire)
            if (TryStrategy5_ForceSync(file, localFilePath, newContent, comment))
            {
                Logger.Log($"[+] Strategie 5 reussie: Force Sync", Logger.LogLevel.WARNING);
                return true;
            }

            Logger.Log($"[-] Toutes les strategies ont echoue pour: {file.Name}", Logger.LogLevel.ERROR);
            return false;
        }

        /// <summary>
        /// STRATEGIE 0 (PRIORITAIRE): CheckoutFile + CheckinFile avec Stream (MemoryStream)
        /// Cette methode evite completement les problemes de chemin local!
        /// Le contenu est passe directement en memoire via un Stream.
        /// </summary>
        private bool TryStrategy0_CheckinWithStream(ACW.File file, string localFilePath, byte[] newContent, string comment)
        {
            ACW.File? checkedOutFile = null;

            try
            {
                Logger.Log($"   [>] Strategie 0: CheckoutFile + CheckinFile(Stream)...", Logger.LogLevel.DEBUG);

                // Checkout via DocumentService
                checkedOutFile = _connection!.WebServiceManager.DocumentService.CheckoutFile(
                    file.Id,
                    ACW.CheckoutFileOptions.Master,
                    Environment.MachineName,
                    localFilePath,  // Chemin pour le placeholder
                    comment,
                    out _);

                if (checkedOutFile == null)
                    throw new Exception("CheckoutFile retourne null");

                Logger.Log($"   [i] Fichier checke out: ID={checkedOutFile.Id}, Ver={checkedOutFile.VerNum}", Logger.LogLevel.DEBUG);

                var checkedOutIteration = new VDF.Vault.Currency.Entities.FileIteration(_connection, checkedOutFile);

                // CheckIn avec Stream - PASSE LE CONTENU DIRECTEMENT EN MEMOIRE!
                using (var contentStream = new MemoryStream(newContent))
                {
                    var checkinResult = _connection.FileManager.CheckinFile(
                        checkedOutIteration,
                        comment,
                        false,           // keepCheckedOut
                        DateTime.UtcNow, // lastWrite
                        null,            // associations
                        null,            // bom
                        false,           // copyBom
                        null,            // newFileName (garder le meme nom)
                        ACW.FileClassification.None,
                        false,           // hidden
                        contentStream    // CONTENU VIA STREAM!
                    );

                    if (checkinResult != null)
                    {
                        Logger.Log($"   [+] CheckinFile(Stream) reussi! Nouvelle version.", Logger.LogLevel.DEBUG);
                        
                        // GET - Telecharger la version fraiche pour enlever le statut rouge (Modified)
                        try
                        {
                            var downloadSettings = new VDF.Vault.Settings.AcquireFilesSettings(_connection, false);
                            downloadSettings.AddFileToAcquire(checkinResult, VDF.Vault.Settings.AcquireFilesSettings.AcquisitionOption.Download);
                            
                            string? destFolder = Path.GetDirectoryName(localFilePath);
                            if (!string.IsNullOrEmpty(destFolder))
                                downloadSettings.LocalPath = new VDF.Currency.FolderPathAbsolute(destFolder);
                            
                            _connection.FileManager.AcquireFiles(downloadSettings);
                            Logger.Log($"   [+] GET effectue - fichier synchronise avec Vault", Logger.LogLevel.DEBUG);
                        }
                        catch (Exception getEx)
                        {
                            Logger.Log($"   [!] GET echoue (non bloquant): {getEx.Message}", Logger.LogLevel.DEBUG);
                            // Non bloquant - le checkin a reussi, c'est juste le statut visuel
                            WriteContentToFile(localFilePath, newContent);
                        }
                        
                        checkedOutFile = null; // Marquer comme traite
                        LogNewVersion(file.MasterId);
                        return true;
                    }
                    else
                    {
                        Logger.Log($"   [-] CheckinFile(Stream) retourne null", Logger.LogLevel.DEBUG);
                        return false;
                    }
                }
            }
            ...
            {
                Logger.Log($"   [-] Strategie 0 echouee: {ex.Message}", Logger.LogLevel.DEBUG);
                if (checkedOutFile != null)
                    SafeUndoCheckout(checkedOutFile.MasterId);
                return false;
            }
        }

        /// <summary>
        /// Strategie 1: AcquireFiles(Checkout) + CheckinFile avec chemin exact de AcquireFiles
        /// </summary>
        private bool TryStrategy1_AcquireCheckout_CheckinWithPath(ACW.File file, string localFilePath, byte[] newContent, string comment)
        {
            string? actualLocalPath = null;
            bool checkedOut = false;

            try
            {
                Logger.Log($"   [>] Strategie 1: AcquireFiles(Checkout) + CheckinFile(path)...", Logger.LogLevel.DEBUG);

                var fileIteration = new VDF.Vault.Currency.Entities.FileIteration(_connection!, file);
                var acquireSettings = new VDF.Vault.Settings.AcquireFilesSettings(_connection!, false);
                acquireSettings.AddFileToAcquire(fileIteration, VDF.Vault.Settings.AcquireFilesSettings.AcquisitionOption.Checkout);
                ...
                if (!string.IsNullOrEmpty(destFolder))
                {
                    if (!Directory.Exists(destFolder))
                        Directory.CreateDirectory(destFolder);
                    acquireSettings.LocalPath = new VDF.Currency.FolderPathAbsolute(destFolder);
                }

                var acquireResult = _connection!.FileManager.AcquireFiles(acquireSettings);

                if (acquireResult.FileResults == null || !acquireResult.FileResults.Any())
                {
                    Logger.Log($"   [-] Strategie 1: AcquireFiles n'a retourne aucun resultat", Logger.LogLevel.DEBUG);
                    return false;
                }

                var fileResult = acquireResult.FileResults.First();
                actualLocalPath = fileResult.LocalPath?.FullPath;

                if (string.IsNullOrEmpty(actualLocalPath))
                {
                    Logger.Log($"   [-] Strategie 1: chemin local null", Logger.LogLevel.DEBUG);
                    return false;
                }

                checkedOut = true;
                Logger.Log($"   [i] Fichier checke out vers: {actualLocalPath}", Logger.LogLevel.DEBUG);

                // Ecrire le nouveau contenu
                WriteContentToFile(actualLocalPath, newContent);

                // Recuperer le fichier checke out
                var latestFiles = _connection.WebServiceManager.DocumentService.GetLatestFilesByMasterIds(new[] { file.MasterId });
                if (latestFiles == null || latestFiles.Length == 0)
                    throw new Exception("GetLatestFilesByMasterIds retourne null");

                var checkedOutFile = latestFiles[0];
                var checkedOutIteration = new VDF.Vault.Currency.Entities.FileIteration(_connection, checkedOutFile);
                var fileInfo = new FileInfo(actualLocalPath);

                // CheckIn avec chemin explicit
                _connection.FileManager.CheckinFile(
                    checkedOutIteration, comment, false, fileInfo.LastWriteTimeUtc,
                    null, null, false, actualLocalPath, ACW.FileClassification.None, false, null);

                checkedOut = false;
                CopyToRequestedPath(actualLocalPath, localFilePath);
                LogNewVersion(file.MasterId);
                return true;
            }
            catch (Exception ex)
            {
                Logger.Log($"   [-] Strategie 1 echouee: {ex.Message}", Logger.LogLevel.DEBUG);
                if (checkedOut)
                    SafeUndoCheckout(file.MasterId);
                return false;
            }
        }

        /// <summary>
        /// Strategie 2: AcquireFiles(Checkout) + CheckinFile avec null (laisse Vault gerer le chemin)
        /// </summary>
        private bool TryStrategy2_AcquireCheckout_CheckinNull(ACW.File file, string localFilePath, byte[] newContent, string comment)
        {
            string? actualLocalPath = null;
            bool checkedOut = false;
            ...
            try
            {
                Logger.Log($"   [>] Strategie 2: AcquireFiles(Checkout) + CheckinFile(null)...", Logger.LogLevel.DEBUG);
                ...
                // CheckIn avec null - laisse Vault gerer
                _connection.FileManager.CheckinFile(
                    checkedOutIteration, comment, false, fileInfo.LastWriteTimeUtc,
                    null, null, false, null, ACW.FileClassification.None, false, null);
            ...
            {
                Logger.Log($"   [-] Strategie 2 echouee: {ex.Message}", Logger.LogLevel.DEBUG);
                if (checkedOut)
                    SafeUndoCheckout(file.MasterId);
                return false;
            }
        ...
        /// <summary>
        /// Strategie 3: DocumentService.CheckoutFile + Ecrire local + CheckinFile avec chemin
        /// </summary>
        private bool TryStrategy3_DocServiceCheckout_CheckinWithPath(ACW.File file, string localFilePath, byte[] newContent, string comment)
        {
            ACW.File? checkedOutFile = null;

            try
            {
                Logger.Log($"   [>] Strategie 3: DocService.Checkout + CheckinFile(path)...", Logger.LogLevel.DEBUG);
                ...
                // Checkout via DocumentService
                checkedOutFile = _connection!.WebServiceManager.DocumentService.CheckoutFile(
                    file.Id, ACW.CheckoutFileOptions.Master, Environment.MachineName,
                    localFilePath, comment, out _);
                ...
                _connection.FileManager.CheckinFile(
                    checkedOutIteration, comment, false, fileInfo.LastWriteTimeUtc,
                    null, null, false, localFilePath, ACW.FileClassification.None, false, null);
            ...
            {
                Logger.Log($"   [-] Strategie 3 echouee: {ex.Message}", Logger.LogLevel.DEBUG);
                if (checkedOutFile != null)
                    SafeUndoCheckout(checkedOutFile.MasterId);
                return false;
            }
        ...
        /// Strategie 4: DocumentService.CheckoutFile + CheckinFile avec null
        /// </summary>
        private bool TryStrategy4_DocServiceCheckout_CheckinNull(ACW.File file, string localFilePath, byte[] newContent, string comment)
        {
            ACW.File? checkedOutFile = null;

            try
            {
                Logger.Log($"   [>] Strategie 4: DocService.Checkout + CheckinFile(null)...", Logger.LogLevel.DEBUG);
            ...
            {
                Logger.Log($"   [-] Strategie 4 echouee: {ex.Message}", Logger.LogLevel.DEBUG);
                if (checkedOutFile != null)
                    SafeUndoCheckout(checkedOutFile.MasterId);
                return false;
            }
        ...
        /// <summary>
        /// Strategie 5: UndoCheckOut + Force AcquireFiles avec ecrasement + CheckIn
        /// Garantit que le fichier local est synchronise avant checkin
        /// </summary>
        private bool TryStrategy5_ForceSync(ACW.File file, string localFilePath, byte[] newContent, string comment)
        {
            try
            {
                Logger.Log($"   [>] Strategie 5: Force Sync (UndoCheckOut + Reacquire)...", Logger.LogLevel.WARNING);

                // D'abord annuler tout checkout existant
                try
                {
                    _connection!.WebServiceManager.DocumentService.UndoCheckoutFile(file.MasterId, out _);
                    Logger.Log($"   [i] UndoCheckOut execute", Logger.LogLevel.DEBUG);
                    System.Threading.Thread.Sleep(500);
                }
                catch (Exception ex)
                {
                    Logger.Log($"   [i] UndoCheckOut: {ex.Message} (peut etre normal)", Logger.LogLevel.DEBUG);
                }

                // Re-obtenir le fichier frais du serveur
                var freshFile = _connection!.WebServiceManager.DocumentService.GetLatestFileByMasterId(file.MasterId);
                if (freshFile == null)
                {
                    Logger.Log($"   [-] Impossible de re-obtenir le fichier", Logger.LogLevel.DEBUG);
                    return false;
                }

                // Creer le FileIteration
                var fileIteration = new VDF.Vault.Currency.Entities.FileIteration(_connection, freshFile);
                ...
                // Acquire avec Checkout
                var acquireSettings = new VDF.Vault.Settings.AcquireFilesSettings(_connection, false);
                acquireSettings.AddFileToAcquire(fileIteration, VDF.Vault.Settings.AcquireFilesSettings.AcquisitionOption.Checkout);
                if (!string.IsNullOrEmpty(destFolder))
                    acquireSettings.LocalPath = new VDF.Currency.FolderPathAbsolute(destFolder);
                
                var results = _connection.FileManager.AcquireFiles(acquireSettings);
                
                if (results == null || results.FileResults == null || !results.FileResults.Any())
                {
                    Logger.Log($"   [-] AcquireFiles n'a retourne aucun resultat", Logger.LogLevel.DEBUG);
                    return false;
                }

                var fileResult = results.FileResults.First();
                string? acquiredPath = fileResult.LocalPath?.FullPath ?? localFilePath;
                
                Logger.Log($"   [i] Fichier acquis a: {acquiredPath}", Logger.LogLevel.DEBUG);

                // Ecrire le nouveau contenu
                WriteContentToFile(acquiredPath, newContent);
                Logger.Log($"   [i] Nouveau contenu ecrit ({newContent.Length} bytes)", Logger.LogLevel.DEBUG);

                // Attendre un peu
                System.Threading.Thread.Sleep(300);

                // Recuperer le fichier apres checkout
                var latestFiles = _connection.WebServiceManager.DocumentService.GetLatestFilesByMasterIds(new[] { file.MasterId });
                if (latestFiles == null || latestFiles.Length == 0)
                {
                    Logger.Log($"   [-] GetLatestFilesByMasterIds retourne null", Logger.LogLevel.DEBUG);
                    return false;
                }
                ...
                // CheckIn avec le chemin acquis (signature 11 arguments)
                var fileInfo = new FileInfo(acquiredPath);
                _connection.FileManager.CheckinFile(
                    checkedOutIteration,
                    comment,
                    false,  // keepCheckedOut
                    ...
                    null,   // bom
                    false,  // copyLocalFile
                    acquiredPath,  // Chemin exact du fichier acquis
                    ACW.FileClassification.None,
                    false,  // hidden
                    null    // dependencies
                );
                ...
                var updatedFiles = _connection.WebServiceManager.DocumentService.GetLatestFilesByMasterIds(new[] { file.MasterId });
                if (updatedFiles != null && updatedFiles.Length > 0 && updatedFiles[0].VerNum > file.VerNum)
                {
                    Logger.Log($"   [+] Strategie 5 REUSSIE! Version: {updatedFiles[0].VerNum}", Logger.LogLevel.INFO);
                    return true;
                }
                else
                {
                    Logger.Log($"   [-] CheckIn retourne null", Logger.LogLevel.DEBUG);
                    return false;
                }
            }
            catch (Exception ex)
            {
                Logger.Log($"   [-] Strategie 5 echouee: {ex.Message}", Logger.LogLevel.DEBUG);
                return false;
            }
        }

        /// <summary>
        /// Helper: Ecrire le contenu dans un fichier (gere ReadOnly)
        /// </summary>
        private void WriteContentToFile(string filePath, byte[] content)
        {
            ...
            Logger.Log($"   [i] Contenu ecrit: {content.Length} bytes -> {filePath}", Logger.LogLevel.DEBUG);
        }

        /// <summary>
        /// Helper: Copier vers le chemin demande si different
        /// </summary>
        private void CopyToRequestedPath(string actualPath, string requestedPath)
        {
            if (!string.Equals(actualPath, requestedPath, StringComparison.OrdinalIgnoreCase))
            {
                try
                {
                    System.IO.File.Copy(actualPath, requestedPath, true);
                    Logger.Log($"   [i] Copie vers: {requestedPath}", Logger.LogLevel.DEBUG);
                }
                catch { /* Ignorer */ }
            }
        }
        ...
        /// <summary>
        /// Helper: UndoCheckout securise
        /// </summary>
        private void SafeUndoCheckout(long masterId)
        {
            try
            {
                _connection?.WebServiceManager.DocumentService.UndoCheckoutFile(masterId, out _);
                Logger.Log($"   [i] UndoCheckOut effectue", Logger.LogLevel.DEBUG);
            }
            catch { /* Ignorer */ }
        }
        ...
        /// <summary>
        /// Helper: Logger la nouvelle version
        /// </summary>
        private void LogNewVersion(long masterId)
        {
            try
            {
                var latestFiles = _connection?.WebServiceManager.DocumentService.GetLatestFilesByMasterIds(new[] { masterId });
                if (latestFiles != null && latestFiles.Length > 0)
                {
                    Logger.Log($"   [i] Nouvelle version: {latestFiles[0].VerNum}", Logger.LogLevel.INFO);
                }
            }
            catch { /* Ignorer */ }
        }
        ...
        public bool CheckInFile(ACW.File checkedOutFile, string localFilePath, string comment)
        {
            ...
            try
            {
                ...
                // Utiliser FileManager.CheckinFile
                var fileIteration = new VDF.Vault.Currency.Entities.FileIteration(_connection, checkedOutFile);
                
                // CheckIn avec le fichier local
                // IMPORTANT: filePathAbsolute = null car le fichier doit etre au meme chemin
                // que celui specifie lors du CheckOut (localFilePath)
                _connection.FileManager.CheckinFile(
                    fileIteration,
                    comment,
                    false,  // keepCheckedOut
                    ...
                    null,   // bom
                    false,  // copyBom
                    null,   // filePathAbsolute - null pour utiliser le fichier au chemin du CheckOut
                    ACW.FileClassification.None,
                    false,  // hidden
                    null    // dependencyOptions
                );
