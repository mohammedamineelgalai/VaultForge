    ...
    public static class PdfFormFillerService
    {
                                              ...
                                              int totalPdfTags)
        {
            ...
            try
            {
                ...
                // Utiliser VaultFileHelper pour gerer ReadOnly automatiquement
                VaultFileHelper.CopyFile(pdfPath, tempPath, true);
                VaultFileHelper.RemoveReadOnly(pdfPath); // S'assurer que l'original est editable
            ...
            finally
            {
                ...
                if (!string.IsNullOrEmpty(tempPath) && File.Exists(tempPath))
                {
                    try
                    {
                        VaultFileHelper.DeleteFile(tempPath);
                    }
                    catch
                    {
                        // Ignorer
