            ...
            try
            {
                ...
                if (!File.Exists(targetFilePath))
                {
                    ...
                    if (existingTemplateFiles.Length > 0)
                    {
                        ...
                        VaultFileHelper.MoveFile(oldFilePath, targetFilePath);
                    ...
                    else if (File.Exists(templateExcelPath))
                    {
                        ...
                        VaultFileHelper.CopyFile(templateExcelPath, targetFilePath);
                        needsProjectCellsUpdate = true;
                ...
                {
                    // S'assurer que le fichier existant est editable
                    VaultFileHelper.RemoveReadOnly(targetFilePath);
                ...
                if (!File.Exists(checkListFilePath))
                {
                    ...
                    if (existingCheckListFiles.Length > 0)
                    {
                        ...
                        VaultFileHelper.MoveFile(oldCheckListPath, checkListFilePath);
                    ...
                    {
                        VaultFileHelper.CopyFile(templateCheckListPath, checkListFilePath);
                        Log("FileIO", $"[+] Check List copiee depuis template: {checkListFileName}");
                    }
                }
                else
                {
                    // S'assurer que la Check List existante est editable
                    VaultFileHelper.RemoveReadOnly(checkListFilePath);
                }
