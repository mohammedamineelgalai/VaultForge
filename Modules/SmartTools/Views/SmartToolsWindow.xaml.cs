...
namespace XnrgyEngineeringAutomationTools.Modules.SmartTools.Views
{
    ...
    public partial class SmartToolsWindow : Window
    {
        ...
        private async void BtnCheckSaveStatus_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                // Obtenir la fenêtre Inventor pour centrer le message
                Window? inventorWindow = GetInventorWindow();
                        ...
                        Dispatcher.Invoke(() =>
                        {
                            ...
                            catch
                            {
                                XnrgyMessageBox.ShowInfo(
                                    "Aucun document n'est actuellement ouvert dans Inventor.",
                                    "Check Save Status - XNRGY",
                                    this);
                                return;
                            }
                            ...
                            try
                            {
                                isDirty = activeDoc.Dirty;
                                string fullPath = activeDoc.FullFileName;
                                if (!string.IsNullOrEmpty(fullPath))
                                {
                                    docName = System.IO.Path.GetFileName(fullPath);
                                }
                                else
                                {
                                    docName = activeDoc.DisplayName ?? "Document sans nom";
                                }
                            }
                            catch (Exception ex)
                            {
                                XnrgyMessageBox.ShowError(
                                    $"Erreur lors de la vérification du document: {ex.Message}",
                                    "Erreur - XNRGY",
                                    this);
                                return;
                            }
                            ...
                            if (isDirty)
                            {
                                // Document non sauvegardé - demander de sauvegarder
                                // [+] Utiliser ConfirmOnInventor pour centrer sur Inventor
                                bool shouldSave = XnrgyMessageBox.ConfirmOnInventor(
                                    $"Le document '{docName}' contient des modifications non sauvegardées.\n\n" +
                                    "Voulez-vous le sauvegarder maintenant?",
                                    "Document non sauvegardé - XNRGY");

                                if (shouldSave)
                                {
                                    try
                                    {
                                        activeDoc.Save2(true);
                                        // [+] Utiliser ShowSuccessOnInventor pour centrer sur Inventor
                                        XnrgyMessageBox.ShowSuccessOnInventor(
                                            $"Le document '{docName}' a été sauvegardé avec succès.",
                                            "Sauvegarde réussie - XNRGY");
                                        Log($"Document '{docName}' sauvegardé", LogLevel.SUCCESS);
                                    }
                                    catch (Exception ex)
                                    {
                                        // [+] Erreur sur l'app XEAT (this = SmartToolsWindow)
                                        XnrgyMessageBox.ShowError(
                                            $"Erreur lors de la sauvegarde du document:\n{ex.Message}",
                                            "Erreur de sauvegarde - XNRGY",
                                            this);
                                        Log($"Erreur sauvegarde: {ex.Message}", LogLevel.ERROR);
                                    }
                                }
                                else
                                {
                                    Log($"Document '{docName}' non sauvegardé (utilisateur a annulé)", LogLevel.INFO);
                                }
                            }
                            else
                            {
                                // Document déjà sauvegardé
                                // [+] Utiliser ShowSuccessOnInventor pour centrer sur Inventor
                                XnrgyMessageBox.ShowSuccessOnInventor(
                                    $"Le document '{docName}' est déjà sauvegardé.\n\n" +
                                    "Aucune modification en attente.",
                                    "Document sauvegardé - XNRGY");
                                Log($"Document '{docName}' est sauvegardé", LogLevel.SUCCESS);
                            }
                        });
                    }
                    catch (Exception ex)
        ...
        // Fenêtre wrapper réutilisable pour Inventor
        private Window? _inventorWrapperWindow = null;

        /// <summary>
        /// Obtient la fenêtre principale d'Inventor pour l'utiliser comme Owner de MessageBox
        /// Crée un Window wrapper invisible basé sur le HWND d'Inventor
        /// Utilise la même approche que les autres fenêtres (ExportOptionsWindow, etc.)
        /// </summary>
        private Window? GetInventorWindow()
        {
            try
            {
                ...
                foreach (var proc in inventorProcesses)
                {
                    ...
                    {
                        // Si la fenêtre wrapper existe déjà, vérifier qu'elle est toujours valide
                        if (_inventorWrapperWindow != null)
                        {
                            try
                            {
                                // Vérifier que le handle est toujours valide
                                var helper = new WindowInteropHelper(_inventorWrapperWindow);
                                if (helper.Handle != IntPtr.Zero && helper.Owner == proc.MainWindowHandle)
                                {
                                    return _inventorWrapperWindow;
                                }
                            }
                            catch
                            {
                                // La fenêtre wrapper n'est plus valide, on va en créer une nouvelle
                                _inventorWrapperWindow = null;
                            }
                        }

                        // Créer une nouvelle fenêtre wrapper invisible
                        // IMPORTANT: Cette fenêtre doit rester en vie pour être utilisée comme owner
                        _inventorWrapperWindow = new Window
                        {
                            ...
                            Width = 1,
                            Height = 1,
                            Left = -10000, // Hors écran
                            Top = -10000,
                            Topmost = false,
                            ShowActivated = false
                        };

                        // Définir le Owner AVANT de montrer la fenêtre (c'est crucial!)
                        var interopHelper = new WindowInteropHelper(_inventorWrapperWindow);
                        
                        // Forcer la création du handle
                        interopHelper.EnsureHandle();
                        
                        // Définir le Owner = handle d'Inventor
                        // Cela permet au MessageBox de s'afficher au centre d'Inventor
                        interopHelper.Owner = proc.MainWindowHandle;

                        // Montrer puis cacher immédiatement pour initialiser correctement
                        // La fenêtre reste en vie mais invisible
                        _inventorWrapperWindow.Show();
                        _inventorWrapperWindow.Hide();
            ...
            {
                // En cas d'erreur, retourner null pour utiliser CenterScreen
                Logger.Log($"[SmartTools] [!] Erreur GetInventorWindow: {ex.Message}", Logger.LogLevel.DEBUG);
                _inventorWrapperWindow = null;
            }
        ...
        private async void BtnDeleteSelectedComponentAndFile_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                await _smartToolsService.ExecuteDeleteSelectedComponentAndFileAsync((msg, level) => Log(msg, level));
            }
            catch (Exception ex)
            {
                Log($"Erreur lors de la suppression: {ex.Message}", LogLevel.ERROR);
            }
        }

        private async void BtnRenameSelectedComponentAndFile_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                await _smartToolsService.ExecuteRenameSelectedComponentAndFileAsync((msg, level) => Log(msg, level));
            }
            catch (Exception ex)
            {
                Log($"Erreur lors du renommage: {ex.Message}", LogLevel.ERROR);
            }
        }
