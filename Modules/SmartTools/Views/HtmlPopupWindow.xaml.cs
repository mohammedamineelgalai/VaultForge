    ...
    public partial class HtmlPopupWindow : Window
    {
        ...
        private async void HtmlPopupWindow_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                string userDataFolder = Path.Combine(
                    ...
                    "XnrgyEngineeringAutomationTools", "WebView2Cache", "HtmlPopup");
            ...
            {
                Logger.Log($"[SmartTools][HtmlPopup] WebView2 non disponible: {ex.Message}", Logger.LogLevel.WARNING);
                ShowFallbackContent();
            }
        }
        ...
        private void CenterOnInventorWindow()
        {
            try
            {
                ...
                if (inventorHandle != IntPtr.Zero)
                {
                    ...
                    Logger.Log($"[SmartTools][HtmlPopup] Fenetre centree sur Inventor: Left={this.Left}, Top={this.Top}", Logger.LogLevel.DEBUG);
                }
                else
                {
                    ...
                    Logger.Log("[SmartTools][HtmlPopup] Inventor non trouve, centrage sur ecran", Logger.LogLevel.DEBUG);
                }
            }
            catch (Exception ex)
            {
                Logger.Log($"[SmartTools][HtmlPopup] Erreur centrage sur Inventor: {ex.Message}", Logger.LogLevel.WARNING);
                this.Left = (SystemParameters.PrimaryScreenWidth - this.Width) / 2;
                this.Top = (SystemParameters.PrimaryScreenHeight - this.Height) / 2;
            }
        }
        ...
        private void CoreWebView2_WebMessageReceived(object? sender, Microsoft.Web.WebView2.Core.CoreWebView2WebMessageReceivedEventArgs e)
        {
            try
            {
                ...
                Logger.Log($"[SmartTools][HtmlPopup][WebView2] Message recu: {message}", Logger.LogLevel.DEBUG);
            ...
            {
                Logger.Log($"[SmartTools][HtmlPopup] Erreur traitement message WebView2: {ex.Message}", Logger.LogLevel.ERROR);
            }
        }
            ...
            System.Collections.Generic.Dictionary<string, string> properties)
        {
            ...
            // Footer
            sb.AppendLine("<div class='footer'>🏷️ Smart iProperties Summary - By Mohammed Amine Elgalai - XNRGY Climate Systems ULC</div>");
            sb.AppendLine("</div>");
