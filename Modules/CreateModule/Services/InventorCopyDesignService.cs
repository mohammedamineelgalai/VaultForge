...
namespace XnrgyEngineeringAutomationTools.Modules.CreateModule.Services
{
    ...
    public class InventorCopyDesignService : IDisposable
    {
        ...
        #region Vault ReadOnly Utilities
        
        /// <summary>
        /// Retire l'attribut ReadOnly d'un fichier s'il est present.
        /// IMPORTANT: Les fichiers Vault sont toujours en ReadOnly apres telechargement.
        /// </summary>
        ...
        /// IMPORTANT: Appeler cette methode AVANT toute operation sur des fichiers Vault.
        /// </summary>
        ...
        public static int RemoveReadOnlyRecursive(string path, Action<string, string>? logCallback = null)
        {
            ...
            try
            {
                ...
                foreach (var file in Directory.GetFiles(path, "*.*", SearchOption.AllDirectories))
                {
                    try
                    {
                        if (RemoveReadOnlyAttribute(file))
                            count++;
                    }
                ...
                if (count > 0)
                {
                    logCallback?.Invoke($"[+] ReadOnly retire de {count} elements dans: {path}", "DEBUG");
                }
            }
            catch (Exception ex)
            {
        ...
        /// Les fichiers Vault sont en lecture seule - il faut les rendre editables.
        /// </summary>
        private static void CopyFileAndRemoveReadOnly(string source, string destination, bool overwrite = true)
        {
            System.IO.File.Copy(source, destination, overwrite);
            RemoveReadOnlyAttribute(destination);
        }
        ...
        private async Task FinalizeNewModuleAsync(string topAssemblyPath, CreateModuleRequest request)
        {
            ...
            await Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    try
                    {
                        // CRITIQUE: Retirer ReadOnly sur tous les fichiers AVANT sauvegarde (fichiers Vault)
                        foreach (Document doc in _inventorApp.Documents)
                        {
                            try
                            {
                                string currentDocPath = doc.FullFileName;
                                if (!string.IsNullOrEmpty(currentDocPath) && VaultFileHelper.IsVaultPath(currentDocPath))
                                {
                                    VaultFileHelper.RemoveReadOnly(currentDocPath);
                                }
                            }
                        ...
                        // Sauvegarder tous les documents ouverts (au cas ou)
                        foreach (Document doc in _inventorApp.Documents)
                        {
                            try
                            {
                    ...
                    if (visibleDoc != null)
                    {
                        ...
                        // 11. NOUVEAU: Save All pour garantir que tous les changements sont sauvegardes
                        try
                        {
                            Log("[>] Sauvegarde finale de tous les documents...", "INFO");
                            SaveAll();
                            Log("[+] Tous les documents sauvegardes", "SUCCESS");
                        }
                        catch (Exception saveEx)
                        {
                            Log($"[!] Erreur sauvegarde finale: {saveEx.Message}", "WARN");
                        }
                    }
                    else
                    {
        ...
        public async Task<ModuleCopyResult> ExecuteCopyDesignAsync(CreateModuleRequest request)
        {
            ...
            try
            {
                ...
                // ═══════════════════════════════════════════════════════════════════════
                // ÉTAPE CRITIQUE: Retirer les attributs ReadOnly des fichiers Vault
                // Les fichiers telecharges depuis Vault sont TOUJOURS en ReadOnly
                // Cela bloque les operations de modification/sauvegarde
                // ═══════════════════════════════════════════════════════════════════════
                Log("[>] Preparation des fichiers Vault (retrait ReadOnly)...", "INFO");
                
                // Retirer ReadOnly du dossier source (template ou projet existant)
                string sourceFolder = System.IO.Path.GetDirectoryName(request.FilesToCopy.FirstOrDefault()?.OriginalPath ?? "") ?? "";
                if (!string.IsNullOrEmpty(sourceFolder) && sourceFolder.StartsWith(@"C:\Vault", StringComparison.OrdinalIgnoreCase))
                {
                    int sourceCount = RemoveReadOnlyRecursive(sourceFolder, Log);
                    if (sourceCount > 0)
                    {
                        Log($"[+] {sourceCount} fichiers source preparés", "DEBUG");
                    }
                }
                
                // Preparer le dossier destination (s'il existe deja)
                if (Directory.Exists(request.DestinationPath))
                {
                    int destCount = RemoveReadOnlyRecursive(request.DestinationPath, Log);
                    if (destCount > 0)
                    {
                        Log($"[+] {destCount} fichiers destination preparés", "DEBUG");
                    }
                }
                ...
                await Task.Run(() =>
                {
                    ...
                    try
                    {
                        ...
                        // ═══════════════════════════════════════════════════════════════
                        // IMPORTANT: Retirer ReadOnly de la destination AVANT modification
                        // Les fichiers copies conservent l'attribut ReadOnly de la source
                        // ═══════════════════════════════════════════════════════════════
                        RemoveReadOnlyRecursive(request.DestinationPath, Log);
                ...
                try
                {
                    ...
                    foreach (var orphanFile in orphanFiles)
                    {
                        try
                        {
                            ...
                            // Copie simple du fichier (retirer ReadOnly car fichiers Vault)
                            CopyFileAndRemoveReadOnly(orphanFile, destPath, true);
            ...
            await Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    foreach (var kvp in sortedModuleFiles)
                    {
                        ...
                        try
                        {
                            // CRITIQUE: Retirer ReadOnly AVANT SaveAs (fichiers Vault)
                            if (VaultFileHelper.IsVaultPath(newPath))
                            {
                                VaultFileHelper.RemoveReadOnly(newPath);
                            }
                            
                            // IMPORTANT: SaveAs met a jour automatiquement les references
                            // car tous les documents sont charges en memoire simultanement
                    ...
                    foreach (var idwPath in idwFiles)
                    {
                        try
                        {
                            ...
                            // ═══════════════════════════════════════════════════════════
                            CopyFileAndRemoveReadOnly(idwPath, newIdwPath, true);
        ...
        private void UpdateAssemblyReferences(string assemblyPath, Dictionary<string, string> pathMapping)
        {
            ...
            try
            {
                ...
                if (modified)
                {
                    ...
                    if (VaultFileHelper.IsVaultPath(assemblyPath))
                    {
                        VaultFileHelper.RemoveReadOnly(assemblyPath);
                    }
            ...
            try
            {
                ...
                foreach (ReferencedFileDescriptor refDesc in doc.ReferencedFileDescriptors)
                {
                    ...
                    try
                    {
                        ...
                        // MÉTHODE 4: Référence externe (Library, etc.) - remapper par nom vers destRoot
                        // Pour les références comme 000000000.iam qui pointent vers Library mais doivent
                        // pointer vers le nouveau fichier renommé (ex: 123450102.iam) dans destRoot
                        else if (renameMapByName.TryGetValue(refFileNameLower, out string? renamedToNew) && !string.IsNullOrEmpty(renamedToNew))
                        {
                            // Le fichier a été renommé - rediriger vers destRoot avec le nouveau nom
                            newPath = System.IO.Path.Combine(destRoot, renamedToNew);
                        }
            ...
            AssemblyDocument asmDoc, string sourceRoot, string destRoot, string newTopAssemblyName)
        {
            ...
            await Task.Run(() =>
            {
                try
                {
                    ...
                    // CRITIQUE: Retirer ReadOnly AVANT SaveAs (fichiers Vault)
                    if (VaultFileHelper.IsVaultPath(topAssemblyNewPath))
                    {
                        VaultFileHelper.RemoveReadOnly(topAssemblyNewPath);
                    }
                ...
                try
                {
                    ...
                    foreach (var drawingFile in drawingFiles)
                    {
                        ...
                        try
                        {
                            ...
                            // Copier le fichier (retirer ReadOnly car fichiers Vault)
                            CopyFileAndRemoveReadOnly(drawingFile, destPath, overwrite: true);
                ...
                try
                {
                    ...
                    foreach (var filePath in allFiles)
                    {
                        ...
                        try
                        {
                            ...
                            // Copier le fichier (retirer ReadOnly car fichiers Vault)
                            CopyFileAndRemoveReadOnly(filePath, destPath, overwrite: true);
        ...
        private void PrepareViewForDesigner(Document doc)
        {
            try
            {
                // 1. Activer la representation par defaut (POSITION 2 prioritaire - comme dans la regle iLogic)
        ...
        /// </summary>
        private void ActivateDefaultRepresentation(Document doc)
        {
            try
            {
                ...
                if (isAssembly)
                {
                    ...
                    DesignViewRepresentations designViewReps = asmDef.RepresentationsManager.DesignViewRepresentations;

                    DesignViewRepresentation? targetRep = null;

                    Log($"  [ActivateDefaultRepresentation] Assemblage - {designViewReps.Count} representations", "DEBUG");

                    // ASSEMBLAGES: POSITION 2 PRIORITAIRE (comme dans la regle iLogic SmartSave)
                    if (designViewReps.Count >= 2)
                    {
                        targetRep = designViewReps[2];
                        Log($"  [+] Position 2 selectionnee: {targetRep.Name}", "DEBUG");
                    }
                    else if (designViewReps.Count >= 1)
                    {
                        targetRep = designViewReps[1];
                        Log($"  [+] Position 1 selectionnee: {targetRep.Name}", "DEBUG");
                    }
                    ...
                    if (targetRep != null)
                    {
                        targetRep.Activate();
                        asmDoc.Update();
                        Log($"  [+] Representation '{targetRep.Name}' activee", "SUCCESS");
                    }
                }
                else if (isPart)
                {
                    ...
                    DesignViewRepresentations designViewReps = partDef.RepresentationsManager.DesignViewRepresentations;

                    DesignViewRepresentation? targetRep = null;
                    
                    Log($"  [ActivateDefaultRepresentation] Piece - {designViewReps.Count} representations", "DEBUG");

                    // PIECES: Recherche mots-cles puis position 2
                    foreach (DesignViewRepresentation rep in designViewReps)
                    {
                        ...
                        {
                            targetRep = rep;
                            Log($"  [+] Representation par mot-cle trouvee: {rep.Name}", "DEBUG");
                            break;
                        }
                    }
            ...
            {
                Log($"  Note: ActivateDefaultRepresentation: {ex.Message}", "DEBUG");
                    ...
                    if (ipjFiles.Length == 0)
                    {
                        ...
                        if (Directory.Exists(sourceTemplatePath))
                        {
                            ...
                            if (templateIpjFiles.Length > 0)
                            {
                                ...
                                // Copier et renommer (retirer ReadOnly car fichiers Vault)
                                CopyFileAndRemoveReadOnly(templateIpj, newIpjPath, true);
        ...
        /// CORRIGE: Utilise Save2(False) pour eviter le dialogue de sauvegarde des dependants
        /// </summary>
        public void SaveAll()
        {
            if (_inventorApp == null) return;
            ...
            try
            {
                ...
                // STRATEGIE: Sauvegarder les Parts (.ipt) d'abord, puis les Assemblages (.iam)
                // Cela evite le dialogue "Save dependents" car les dependants sont deja sauvegardes
                var docList = new System.Collections.Generic.List<Document>();
                foreach (Document doc in documents)
                {
                    docList.Add(doc);
                }
                
                // Trier: Parts d'abord, puis Assemblages, puis Drawings
                var sortedDocs = docList
                    .Where(d => d.IsModifiable)
                    .OrderBy(d => 
                    {
                        string ext = System.IO.Path.GetExtension(d.FullFileName).ToLowerInvariant();
                        if (ext == ".ipt") return 0; // Parts en premier
                        if (ext == ".iam") return 1; // Assemblages ensuite
                        if (ext == ".idw" || ext == ".dwg") return 2; // Drawings en dernier
                        return 3;
                    })
                    .ToList();
                ...
                foreach (Document doc in sortedDocs)
                {
                    try
                    {
                        ...
                        // CORRECTION: Utiliser Save2(False) pour sauvegarder SANS dialogue pour les dependants
                        // Le parametre False = ne pas demander de sauvegarder les fichiers references modifies
                        // (on les sauvegarde deja individuellement dans l'ordre correct)
                        doc.Save2(false);
                        savedCount++;
                    }
                    catch { }
