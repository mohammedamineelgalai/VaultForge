        ...
        private async Task<List<FileCopyResult>> CopyFilesAsync(CreateModuleRequest request, List<FileRenameItem> files)
        {
            ...
            foreach (var file in files)
            {
                ...
                try
                {
                    ...
                    // Copier le fichier et retirer automatiquement ReadOnly
                    await Task.Run(() => VaultFileHelper.CopyFile(file.OriginalPath, newPath, true));
                    ...
                    Log($"Copie: {file.OriginalFileName} -> {file.NewFileName}", "INFO");
                }
                catch (Exception ex)
                {
